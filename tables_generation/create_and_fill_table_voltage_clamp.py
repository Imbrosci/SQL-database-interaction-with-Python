...
Create and fill a table with the voltage clamp data from neurons from the
entorhinal cortex. The modulation of firing by SWRs is also included in the
table.
...
create_table_voltage_clamp = '''
CREATE TABLE voltage_clamp(
...
cur.execute(create_table_voltage_clamp)
conn.commit()

# %% load the file with the values and organize the values as a list of tuples

filepath = '''C:/Users/imbroscb/Desktop/science work/SWRs projects/manuscript 2/analysis for manuscript 2/sql/voltage_clamp_for_SQL.xlsx'''
...
insert_query = ''' INSERT INTO voltage_clamp(
    date_of_recording,
    neuron_number,
    slice_number,
    layer,
...
cur.execute('SELECT * FROM voltage_clamp')
...
cur.execute(
    'ALTER TABLE voltage_clamp ADD COLUMN firing_modulation VARCHAR(19)')
conn.commit()

# %% add the values in the column firing_modulation

update_query = '''UPDATE voltage_clamp
