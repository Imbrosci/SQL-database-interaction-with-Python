...
clamp data from neurons from the entorhinal cortex.
...
create_table_PSPs_SWRs_coupled = '''
CREATE TABLE psps_swrs_coupled(
...
cur.execute(create_table_PSPs_SWRs_coupled)
conn.commit()
...
insert_query = ''' INSERT INTO psps_swrs_coupled(
    date_of_recording,
    neuron_number,
    slice_number,
    layer,
...
cur.execute('SELECT * FROM psps_swrs_coupled')
